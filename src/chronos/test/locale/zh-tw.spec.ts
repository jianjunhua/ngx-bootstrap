...
describe('locale: zh-cn', () => {
  ...
  it('from', function () {
    ...
    assertEq(start.from(moment([2007, 1, 28]).add({ s: 45 }), true), '1 分鐘', '45 seconds = a minute');
    assertEq(start.from(moment([2007, 1, 28]).add({ s: 89 }), true), '1 分鐘', '89 seconds = a minute');
    assertEq(start.from(moment([2007, 1, 28]).add({ s: 90 }), true), '2 分鐘', '90 seconds = 2 minutes');
    assertEq(start.from(moment([2007, 1, 28]).add({ m: 44 }), true), '44 分鐘', '44 minutes = 44 minutes');
    assertEq(start.from(moment([2007, 1, 28]).add({ m: 45 }), true), '1 小時', '45 minutes = an hour');
    assertEq(start.from(moment([2007, 1, 28]).add({ m: 89 }), true), '1 小時', '89 minutes = an hour');
    assertEq(start.from(moment([2007, 1, 28]).add({ m: 90 }), true), '2 小時', '90 minutes = 2 hours');
    assertEq(start.from(moment([2007, 1, 28]).add({ h: 5 }), true), '5 小時', '5 hours = 5 hours');
    assertEq(start.from(moment([2007, 1, 28]).add({ h: 21 }), true), '21 小時', '21 hours = 21 hours');
  ...
  it('suffix', function () {
    assertEq(moment(30000).from(0), '几秒內', 'prefix');
  ...
  it('fromNow', function () {
    assertEq(moment().add({ s: 30 }).fromNow(), '几秒內', 'in a few seconds');
    assertEq(moment().add({ d: 5 }).fromNow(), '5 天內', 'in 5 days');
  });
  ...
  it('weeks year starting sunday format', function () {
    assertEq(moment([2012, 0, 1]).format('w ww wo'), '52 52 52週', 'Jan  1 2012 应该是第52週');
    assertEq(moment([2012, 0, 7]).format('w ww wo'), '1 01 1週', 'Jan  7 2012 应该是第 1週');
    assertEq(moment([2012, 0, 14]).format('w ww wo'), '2 02 2週', 'Jan 14 2012 应该是第 2週');
  });
});
